...
public class SubscriptionController {
	...
	public HashMap<String, String> unsubscribe(@RequestBody Object[] arr) {
		//arr at index 0 to string to grab the substring from '=' to the end of the string
	    //2 Object arrays are passed as a requestbody and the owner username and company username is in each of these respectively
		String ownername = arr[0].toString().substring(arr[0].toString().indexOf('=')+1, arr[0].toString().length()-1);
	    String cmpname = arr[1].toString().substring(arr[1].toString().indexOf('=')+1,arr[1].toString().length()-1);
